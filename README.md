...
| Claude Code | `dot_claude/settings.json`；终端 API 变量见 `30-claude.zsh.tmpl`（KeePassXC） | [docs/claude.md](docs/claude.md) |
| Zsh | `dot_zshrc` + `conf.d/`；Claude token/URL 见 `30-claude.zsh.tmpl`（KeePassXC） | [docs/zsh.md](docs/zsh.md) |
...
```json
{
  "api_key": "{{ (keepassxc \"MyApp\").Password }}"
}
```

Claude Code：`dot_claude/settings.json`；终端里 Claude API 变量由 `conf.d/30-claude.zsh.tmpl`（KeePassXC）生成，见 [docs/zsh.md](docs/zsh.md)、[docs/claude.md](docs/claude.md)。
